...
void calculate_schDt_lin3(CustomComplex& schDt_lin3, CustomComplex* sch2Di, bool flag_occ, int freqevalmin, double *ekq, int iw, int freqevalstep, double cedifft_zb_right, double cedifft_zb_left, CustomComplex schDt_left, CustomComplex schDt_lin2, int n1, double pref_zb, CustomComplex pref_zb_compl, CustomComplex schDt_avg)
{
...
//
inline void ssxDittt_kernel(int *inv_igp_index, int *indinv, CustomComplex *aqsmtemp, CustomComplex *aqsntemp, double *vcoul, CustomComplex *I_eps_array, CustomComplex &ssxDittt, int ngpown, int ncouls, int n1,int ifreq, double fact1, double fact2)
{
...
void achsDtemp_Kernel(int number_bands, int ngpown, int ncouls, int *inv_igp_index, int *indinv, CustomComplex *aqsntemp, CustomComplex *aqsmtemp, CustomComplex *I_epsR_array, double *vcoul, CustomComplex &achsDtemp)
{
    ...
    achsDtemp = CustomComplex (achsDtemp_re, achsDtemp_im) ;

}
//
inline void asxDtemp_Kernel(int nvband, int nfreqeval, int ncouls, int ngpown, int nFreq, double freqevalmin, double freqevalstep, double occ, double *ekq, double *dFreqGrid, int *inv_igp_index, int *indinv, CustomComplex *aqsmtemp, CustomComplex *aqsntemp, double *vcoul, CustomComplex *I_epsR_array, CustomComplex *I_epsA_array, CustomComplex *asxDtemp)
{
...
void achDtemp_Kernel(int number_bands, int nvband, int nfreqeval, int ncouls, int ngpown, int nFreq, double freqevalmin, double freqevalstep, double *ekq, double pref_zb, double *pref, double *dFreqGrid, CustomComplex *dFreqBrd, CustomComplex *schDt_matrix, CustomComplex *schDi, CustomComplex *schDi_cor, CustomComplex *sch2Di, CustomComplex *achDtemp)
{
    ...
    for(int n1 = 0; n1 < number_bands; ++n1)
    {
        ...
        {
            flag_occ = n1 < nvband;
            CustomComplex schDt = schDt_matrix[n1*nFreq + ifreq];
            double cedifft_zb = dFreqGrid[ifreq];
            double cedifft_zb_right, cedifft_zb_left;
            CustomComplex schDt_right, schDt_left, schDt_avg, schDt_lin, schDt_lin2, schDt_lin3;
            CustomComplex cedifft_compl(cedifft_zb, 0.00);
            CustomComplex cedifft_cor;
            CustomComplex cedifft_coh = cedifft_compl - dFreqBrd[ifreq];
            CustomComplex pref_zb_compl(0.00, pref_zb);
            ...
            for(int iw = 0; iw < nfreqeval; ++iw)
            {
                ...
                CustomComplex tmp(0.00, pref[ifreq]);
...
inline void achDtemp_cor_Kernel(int number_bands, int nvband, int nfreqeval, int ncouls, int ngpown, int nFreq, double freqevalmin, double freqevalstep, double *ekq, double *dFreqGrid, int *inv_igp_index, int *indinv, CustomComplex *aqsmtemp, CustomComplex *aqsntemp, double *vcoul, CustomComplex *I_epsR_array, CustomComplex *I_epsA_array, CustomComplex *schDi_cor, CustomComplex *schDi_corb, CustomComplex *sch2Di, CustomComplex *ach2Dtemp, CustomComplex *achDtemp_cor, CustomComplex *achDtemp_corb)
{
    bool flag_occ;
    CustomComplex expr0(0.00, 0.00);
    for(int n1 = 0; n1 < number_bands; ++n1)
...
int main(int argc, char** argv)
{
    ...
    CustomComplex expr0( 0.0 , 0.0);
    CustomComplex expr( 0.5 , 0.5);
    CustomComplex expR( 0.5 , 0.5);
    CustomComplex expA( 0.5 , -0.5);
    CustomComplex exprP1( 0.5 , 0.1);
...
//Start to allocate the data structures;
    ...
    mem_alloc += (number_bands * ncouls * sizeof(CustomComplex));

    CustomComplex *aqsmtemp= new CustomComplex[number_bands * ncouls];
    mem_alloc += (number_bands * ncouls * sizeof(CustomComplex));

    CustomComplex *I_epsR_array = new CustomComplex[nFreq * ngpown * ncouls];
    mem_alloc += (nFreq * ngpown * ncouls * sizeof(CustomComplex));

    CustomComplex *I_epsA_array = new CustomComplex[nFreq * ngpown * ncouls];
    mem_alloc += (nFreq * ngpown * ncouls * sizeof(CustomComplex));

    CustomComplex *schDi = new CustomComplex[nfreqeval];
    CustomComplex *sch2Di = new CustomComplex[nfreqeval];
    CustomComplex *schDi_cor = new CustomComplex[nfreqeval];
    CustomComplex *schDi_corb = new CustomComplex[nfreqeval];
    CustomComplex *achDtemp = new CustomComplex[nfreqeval];
    CustomComplex *ach2Dtemp = new CustomComplex[nfreqeval];
    CustomComplex *achDtemp_cor = new CustomComplex[nfreqeval];
    CustomComplex *achDtemp_corb = new CustomComplex[nfreqeval];
    CustomComplex *asxDtemp = new CustomComplex[nfreqeval];
    CustomComplex *dFreqBrd = new CustomComplex[nFreq];
    mem_alloc += (nfreqeval * 9 * sizeof(CustomComplex));
    mem_alloc += (nFreq * sizeof(CustomComplex)) ;

    CustomComplex *schDt_matrix = new CustomComplex[number_bands * nFreq];
    mem_alloc += (nFreq * number_bands * sizeof(CustomComplex));
    ...
    CustomComplex *d_aqsmtemp, *d_aqsntemp, \
    ...
    CudaSafeCall(cudaMallocManaged((void**) &d_aqsmtemp, number_bands*ncouls*sizeof(CustomComplex)));
    CudaSafeCall(cudaMallocManaged((void**) &d_aqsntemp, number_bands*ncouls*sizeof(CustomComplex)));
    CudaSafeCall(cudaMallocManaged((void**) &d_I_epsR_array, nFreq*ngpown*ncouls*sizeof(CustomComplex)));
    CudaSafeCall(cudaMallocManaged((void**) &d_I_epsA_array, nFreq*ngpown*ncouls*sizeof(CustomComplex)));
    CudaSafeCall(cudaMallocManaged((void**) &d_schDt_matrix, number_bands*nFreq*sizeof(CustomComplex)));
    ...
    CudaSafeCall(cudaMallocManaged((void**) &d_ach2Dtemp, nFreq*sizeof(CustomComplex)));
    ...
    CudaSafeCall(cudaMallocManaged((void**) &d_achDtemp_corb, nfreqeval*sizeof(CustomComplex)));

    //Output
    CudaSafeCall(cudaMallocManaged((void**) &d_achsDtemp, sizeof(CustomComplex)));
    ...
    CustomComplex *ssxDittt = new CustomComplex;
    ...
    CudaSafeCall(cudaMemcpy(d_I_epsR_array, I_epsR_array, nFreq*ngpown*ncouls*sizeof(CustomComplex), cudaMemcpyHostToDevice));
    CudaSafeCall(cudaMemcpy(d_I_epsA_array, I_epsA_array, nFreq*ngpown*ncouls*sizeof(CustomComplex), cudaMemcpyHostToDevice));
    CudaSafeCall(cudaMemcpy(d_schDt_matrix, schDt_matrix, number_bands*nFreq*sizeof(CustomComplex), cudaMemcpyHostToDevice));
    ...
    CudaSafeCall(cudaMemcpy(d_achDtemp_corb, achDtemp_corb, nfreqeval*sizeof(CustomComplex), cudaMemcpyHostToDevice));
...
#if !CUDAVER
    CustomComplex achsDtemp(0.00, 0.00);
...
//    /***********achDtemp Kernel ****************/
    ...
    CustomComplex achsDtemp(*achsDtemp_re, *achsDtemp_im);
    ...
    for(int iw = 0; iw < nfreqeval; ++iw)
    {
        asxDtemp[iw] = CustomComplex(asxDtemp_re[iw], asxDtemp_im[iw]);
        achDtemp_cor[iw] = CustomComplex(achDtemp_cor_re[iw], achDtemp_cor_im[iw]);
    }
