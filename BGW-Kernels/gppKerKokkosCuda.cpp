...
//Host Views 
    ...
    ViewVectorTypeComplex::HostMirror h_wtilde_array = Kokkos::create_mirror_view(wtilde_array);
    ViewVectorTypeComplex::HostMirror h_I_eps_array = Kokkos::create_mirror_view(I_eps_array);
    ViewVectorTypeComplex::HostMirror h_asxtemp = Kokkos::create_mirror_view(asxtemp);

    ViewVectorTypeDouble::HostMirror h_wx_array = Kokkos::create_mirror_view(wx_array);
   ...
   for(int i=0; i<ngpown; i++)
       for(int j=0; j<ncouls; j++)
       {
           h_I_eps_array(i*ncouls+j) = expr;
           h_wtilde_array(i*ncouls+j) = expr;
       }
...
//
    for(int ig=0; ig < ngpown; ++ig)
        h_inv_igp_index(ig) = (ig+1) * ncouls / ngpown;

    //Do not know yet what this array represents
    for(int ig=0, tmp=1; ig<ncouls; ++ig,tmp++)
        h_indinv(ig) = ig;
        h_indinv(ncouls) = ncouls-1;
       ...
       for(int iw=nstart; iw<nend; ++iw)
       {
        ...
        for(int iw=nstart; iw<nend; ++iw)
        {
            h_wx_array(iw) = e_lk - e_n1kq + dw*((iw+1)-2);
            if(h_wx_array(iw) < to1) h_wx_array(iw) = to1;
        }
...
//Copy From host to device Views Kokkos::deep_copy( y, h_y )
    ...
    Kokkos::deep_copy(wtilde_array, h_wtilde_array);
    Kokkos::deep_copy(I_eps_array, h_I_eps_array);
    Kokkos::deep_copy(asxtemp, h_asxtemp);
    Kokkos::deep_copy(wx_array, h_wx_array);
