...
__device__  void ncoulsKernel( GPUComplex *wtilde_array, GPUComplex *aqsntemp, GPUComplex *I_eps_array, int ncouls, double wxt, double& achtemp_re_iw, double& achtemp_im_iw, int my_igp, GPUComplex mygpvar1, int n1, double vcoul_igp)
{
    for(int ig = 0; ig < ncouls; ++ig)
    {
        ...
        double achtemp_re_loc = d_GPUComplex_real(\
            d_GPUComplex_mult(\
            d_GPUComplex_mult(\
        ...
        double achtemp_im_loc = d_GPUComplex_imag(\
            d_GPUComplex_mult(\
            d_GPUComplex_mult(\
            d_GPUComplex_product(d_GPUComplex_product(mygpvar1, aqsntemp[n1*ncouls+ig]),\
            d_GPUComplex_product(d_GPUComplex_mult(d_GPUComplex_product(wtilde_array[my_igp*ncouls+ig], d_GPUComplex_conj(wdiff)), rden), I_eps_array[my_igp*ncouls+ig])),\
            ...
            achtemp_re_iw += achtemp_re_loc;
            achtemp_im_iw += achtemp_im_loc;

//        atomicAdd(&achtemp_re_iw , achtemp_re);
//        atomicAdd(&achtemp_im_iw , achtemp_im );
    }
}
