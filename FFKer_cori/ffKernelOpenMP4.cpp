...
#pragma omp target enter data map(alloc: aqsmtemp[0:number_bands*ncouls],aqsntemp[0:number_bands*ncouls], vcoul[0:ncouls], inv_igp_index[0:ngpown], indinv[0:ncouls], I_epsR_array[0:nFreq*ngpown*ncouls], I_epsA_array[0:nFreq*ngpown*ncouls] )

#pragma omp target update to(aqsmtemp[0:number_bands*ncouls],aqsntemp[0:number_bands*ncouls], vcoul[0:ncouls], inv_igp_index[0:ngpown], indinv[0:ncouls], I_epsR_array[0:nFreq*ngpown*ncouls], I_epsA_array[0:nFreq*ngpown*ncouls] )
...
#pragma omp target teams distribute parallel for collapse(2) default(shared)
    for(int n1 = 0; n1 < number_bands; ++n1)
    {
        for(int my_igp = 0; my_igp < ngpown; ++my_igp)
        {
            ...
            GPUComplex achsDtemp_loc = GPUComplex_mult(schsDtemp , vcoul[igp] * 0.5);

            #pragma omp atomic
            achsDtemp.re += GPUComplex_real(achsDtemp_loc);
            #pragma omp atomic
...
#pragma omp target exit data map(delete:aqsmtemp[0:number_bands*ncouls],aqsntemp[0:number_bands*ncouls], vcoul[0:ncouls], inv_igp_index[0:ngpown], indinv[0:ncouls], I_epsR_array[0:nFreq*ngpown*ncouls], I_epsA_array[0:nFreq*ngpown*ncouls])
    cout << "achsDtemp = " ;
    achsDtemp.print();
    cout << "asxDtemp = " ;
    asxDtemp[0].print();
