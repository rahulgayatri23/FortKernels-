            ...
            for(int iw=nstart; iw<nend; ++iw)
            {
               sch_array[iw] = expr0;
                for(int ig = 0; ig<ncouls; ++ig)
                {
                    ...
                    sch_array[iw] = GPUComplex_mult(GPUComplex_product(GPUComplex_product(GPUComplex_conj(aqsmtemp[n1*ncouls+igp]), aqsntemp[n1*ncouls+ig]), GPUComplex_product(delw , I_eps_array[my_igp*ncouls+ig])), 0.5*vcoul[igp]);
                    achtemp_re_loc[iw] += GPUComplex_real(sch_array[iw]);
                    achtemp_im_loc[iw] += GPUComplex_imag(sch_array[iw]);
