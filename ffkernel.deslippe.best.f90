...
integer :: n1
integer :: ispin, iw, ifreq, ijk, iwlda, igt, igtt, iglimit
...
! ngpown = ncouls / (number of mpi tasks)
      ...
      write(6,*)"allocating I_epsR_array, size = ",((1.0*ncouls)*ngpown*nFreq*16)/(1024**2)," Mbytes"
...
!        allocate(tempz(ngpown,nfreq,number_bands))
!
!        call zgemm('t','n',ngpown*nfreq,number_bands,ncouls,(1D0,0D0),I_epsR_array(1,1,1),ngpown*nfreq,aqsntemp(1,1),ncouls,(0D0,0D0),tempz,ngpown*nfreq)
!
!!$OMP PARALLEL do private (my_igp,igp,indigp,igmax,ig,schDtt,I_epsRggp_int, &
!!$OMP                      I_epsAggp_int,schD,schDt,ifreq) reduction(+:schdt_matrix)
!        do n1=1,number_bands
!        do ifreq=1,nFreq
!          schDtt = (0D0,0D0)
!          do my_igp = 1, ngpown
!            indigp = inv_igp_index(my_igp)
!            igp = indinv(indigp)
!            schDtt = schDtt + tempz(my_igp,ifreq,n1) * CONJG(aqsmtemp(igp,n1))
!          enddo
...
!              do igtt = 1, iglimit
!                ig = igt + igtt - 1	
!                schDtt = schDtt + aqsntemp(ig,n1) * CONJG(aqsmtemp(igp,n1)) * I_epsR_array(ig,my_igp,ifreq)
!              enddo
!              schdt_matrix(ifreq,n1) = schdt_matrix(ifreq,n1) + schDtt
!      enddo
!            enddo
