...
#pragma omp target teams distribute num_teams(number_bands) shared(inv_igp_index, indinv, aqsmtemp, aqsntemp, vcoul, I_epsR_array) \
    map(to:inv_igp_index[0:ngpown], indinv[0:ncouls], aqsmtemp[0:number_bands*ncouls], aqsntemp[0:number_bands*ncouls], I_epsR_array[0:nFreq*ngpown*ncouls], vcoul[0:ncouls], schsDtemp) \
    ...
    for(int n1 = 0; n1 < number_bands; ++n1)
    {
        for(int my_igp = 0; my_igp < ngpown; ++my_igp)
        {
            int indigp = inv_igp_index[my_igp];
            int igp = indinv[indigp];
            double schsDtemp_re = 0.00, schDtemp_im = 0.00;


            for(int ig = 0; ig < ncouls; ++ig)
                schsDtemp_re = GPUComplex_real(aqsntemp[n1*ncouls + ig]);
...
#pragma omp target enter data map(alloc: inv_igp_index[0:ngpown], indinv[0:ncouls], aqsmtemp[0:number_bands*ncouls], aqsntemp[0:number_bands*ncouls], I_epsR_array[0:nFreq*ngpown*ncouls], I_epsA_array[0:nFreq*ngpown*ncouls], vcoul[0:ncouls], \
...
#pragma omp target exit data map(delete: inv_igp_index[:0], indinv[:0], aqsmtemp[:0], aqsntemp[:0], I_epsR_array[:0], I_epsA_array[:0], vcoul[:0], \
            ekq[:0], dFreqGrid[:0], asxDtemp[:0], schDi_cor[:0], schDi_corb[:0], sch2Di[:0], ach2Dtemp[:0], achDtemp_corb[:0])
