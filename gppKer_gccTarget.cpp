...
inline void reduce_achstemp(int n1, int number_bands, int* inv_igp_index, int ncouls, std::complex<double> *aqsmtemp, std::complex<double> *aqsntemp, std::complex<double> *I_eps_array, std::complex<double>* achstemp,  int* indinv, int ngpown, double* vcoul, int numThreads)
{
    double to1 = 1e-6;
...
#pragma omp target map(to:inv_igp_index[0:ngpown*ncouls], indinv[0:ncouls], vcoul[0:ncouls],ssx_array[0:3], aqsmtemp[0:number_bands*ncouls], wtilde_array[0:ngpown*ncouls], aqsntemp[0:number_bands*ncouls], I_eps_array[0:ngpown*ncouls]) map(tofrom:asxtemp[0:(nend-nstart)], achtemp[0:(nend-nstart)], acht_n1_loc[0:number_bands] )
{
...
#pragma omp teams distribute shared(vcoul, aqsntemp, aqsmtemp, I_eps_array, ssx_array) firstprivate(sch_array) 
        for(int my_igp=0; my_igp<ngpown; ++my_igp)
        {
            tid = omp_get_thread_num();
