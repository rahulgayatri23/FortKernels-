            ...
            for(int iw=nstart; iw<nend; ++iw)
            {
                ...
                for(int ig = 0; ig<ncouls; ++ig)
                {
                    ...
                    double rden = 1/(wdiff.x*wdiff.x + wdiff.y*wdiff.y);
                     ...
                     delw.x = (wtilde_array[my_igp*ncouls+ig].re * wdiff.x + wtilde_array[my_igp*ncouls+ig].im * wdiff.y) * rden;
                     delw.y = (-wtilde_array[my_igp*ncouls+ig].re * wdiff.y + wtilde_array[my_igp*ncouls+ig].im * wdiff.x) * rden;

                    wdiff.x = delw.x * I_eps_array[my_igp*ncouls + ig].re - delw.y * I_eps_array[my_igp*ncouls +ig].im;
                    wdiff.y = delw.y * I_eps_array[my_igp*ncouls + ig].im + delw.y * I_eps_array[my_igp*ncouls +ig].re;
                    ...
                    sch[iw].x += delw.x*wdiff.x - delw.y*wdiff.y;
                    sch[iw].y += delw.x*wdiff.y + delw.y*wdiff.x;
                }
#pragma omp atomic
                achtemp_re[iw] += sch[iw].x * 0.5 * vcoul[igp];
#pragma omp atomic
                achtemp_im[iw] += sch[iw].y * 0.5 * vcoul[igp];
            }
