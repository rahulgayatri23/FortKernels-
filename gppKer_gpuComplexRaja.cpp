...
typedef RAJA::omp_parallel_for_exec omp_parallel_for_exec_policy;
typedef RAJA::seq_exec seq_exec_policy;
typedef RAJA::omp_reduce reduce_exec_policy;
...
#pragma omp parallel shared(numThreads) private(tid)
   ...
   for(int i=0; i<ncouls; i++)
    ...
    //Raja reduction vars
    RAJA::ReduceSum<reduce_exec_policy, double> achtemp_re0(0.00);
    RAJA::ReduceSum<reduce_exec_policy, double> achtemp_re1(0.00);
    RAJA::ReduceSum<reduce_exec_policy, double> achtemp_re2(0.00);
    RAJA::ReduceSum<reduce_exec_policy, double> achtemp_im0(0.00);
    RAJA::ReduceSum<reduce_exec_policy, double> achtemp_im1(0.00);
    RAJA::ReduceSum<reduce_exec_policy, double> achtemp_im2(0.00);

    auto startKernelTimer = std::chrono::high_resolution_clock::now();

       RAJA::forallN<collapse3_exec_policy, int, int, int>(
            RangeSegment(0, nvband),
            RangeSegment(0, ngpown),
            RangeSegment(nstart, nend),
            [=] RAJA_DEVICE(int n1, int my_igp, int iw)
       {
                    int indigp = inv_igp_index[my_igp];
                    int igp = indinv[indigp];
    ...
    RAJA::forall<omp_parallel_for_exec_policy>(0, number_bands, [=] (int n1)
    {
