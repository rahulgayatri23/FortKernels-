...
!!!$omp target data map(to:vcoul, my_igp, igp, indigp, igmax, mygpvar1, mygpvar2, ssx_array, sch_array, ig, wtilde, wtilde2, halfinvwtilde, ssxcutoff, matngmatmgp, matngmpatmg, sch, ssx, iw, delw, delw2, Omega2, sct, ssxt, wxt, rden, cde, ssxa, scha, delwr, wdiffr, occfact, igend, igbeg, aqsmtemp, aqsntemp, I_eps_array)  map(tofrom:flag_occ, achtemp, asxtemp, acht_n1_loc)

!$omp target data map(to:vcoul, my_igp, igp, indigp, igmax, mygpvar1, mygpvar2, ssx_array, sch_array, ig, wtilde, wtilde2, sch, ssx, ssxcutoff) &
!$omp map(tofrom: achtemp)  
!$omp target
...
!
!!!$pragma omp distribute private (my_igp,igp,indigp,igmax,mygpvar1,mygpvar2,ssx_array,sch_array,ig, wtilde,wtilde2,halfinvwtilde,ssxcutoff,matngmatmgp,matngpmatmg,sch,ssx, iw,delw,delw2,Omega2,scht,ssxt,wxt,  rden,cden,ssxa,scha,delwr,wdiffr,occfact,igend,igbeg) reduction(+:asxtemp,achtemp) schedule(dynamic)
!        do my_igp = 1, ngpown
!
!          indigp = inv_igp_index(my_igp)
!          igp = indinv(indigp)
